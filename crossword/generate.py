...
class CrosswordCreator():
    ...
    def save(self, assignment, filename, PIL=None):
        """
    ...
    def enforce_node_consistency(self):
        ...
        """
        # For each variable (slot) in the crossword
        for var in self.domains:
            # Iterate over the current domain of words
    ...
    def ac3(self, arcs=None):
        ...
        """
        if arcs is None:
            queue = deque([x,y] for x in self.domains for y in self.crossword.neighbors(x))
        else:
            queue = arcs
